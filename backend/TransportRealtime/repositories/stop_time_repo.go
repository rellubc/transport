...
// func (r *StopTimeRepository) GetVehicleStopTimes(vehicleId string, vehicleLon float64, vehicleLat float64) ([]models.TripRealtimeStopTime, error) {
// 	query := `
// 		WITH stop_data AS (
// 			SELECT
// 				COALESCE(NULLIF(st.stop_headsign, ''), t.trip_headsign, 'No headsign') AS trip_headsign,
...
// 				) AS consist
// 			FROM trips t
// 			JOIN vehicle_positions vp ON t.trip_id = vp.trip_id
// 			JOIN stop_times st ON t.trip_id = st.trip_id
// 			LEFT JOIN stop_time_updates stu ON st.stop_id = stu.stop_id AND st.trip_id = stu.trip_id
// 			JOIN stops s ON st.stop_id = s.stop_id
// 			JOIN routes r ON t.route_id = r.route_id
...
func (r *StopTimeRepository) GetStopRealtimeStopTimesV2(stopId string, direction string, time int) ([]models.StopRealtimeStopTimeV2, error) {
	baseQuery := `
		WITH now_sydney AS MATERIALIZED (
			SELECT
				(NOW() AT TIME ZONE 'Australia/Sydney')::date AS now_date,
		...
		active_trips AS MATERIALIZED (
			SELECT t.trip_id, t.trip_headsign, r.route_short_name, r.route_colour
			FROM trips t
			JOIN active_services ac ON ac.service_id = t.service_id
			JOIN routes r ON t.route_id = r.route_id
		),
		candidates AS MATERIALIZED (
			SELECT DISTINCT ON (t.trip_id)
				...
				CASE
					...
					ELSE st.departure_time + COALESCE(stu.stop_departure_delay, 0)
				END AS display_time,
				(stu.stop_departure_delay IS NOT NULL) AS is_realtime
			FROM stop_times st
			JOIN stops s ON st.stop_id = s.stop_id
	...
	var sts = []models.StopRealtimeStopTimeV2{}
	for rows.Next() {
		var st models.StopRealtimeStopTimeV2
		err := rows.Scan(
			&st.TripId,
			&st.TripHeadsign,
			&st.RouteShortName,
...
func (r *StopTimeRepository) GetTripRealtimeStopTimesV2(vehicleId string, vehicleLon float64, vehicleLat float64) ([]models.TripRealtimeStopTime, error) {
	query := `
		WITH stop_data AS (
			SELECT
				COALESCE(NULLIF(st.stop_headsign, ''), t.trip_headsign, 'No headsign') AS trip_headsign,
			...
			FROM trips t
			LEFT JOIN vehicle_positions vp ON t.trip_id = vp.trip_id
			JOIN stop_times st ON t.trip_id = st.trip_id
			LEFT JOIN stop_time_updates stu ON st.stop_id = stu.stop_id AND st.trip_id = stu.trip_id
			JOIN stops s ON st.stop_id = s.stop_id
			JOIN routes r ON t.route_id = r.route_id
...
func (r *StopTimeRepository) GetVehicleRealtimeStopTimesV2(vehicleId string, vehicleLon float64, vehicleLat float64) ([]models.TripRealtimeStopTime, error) {
	query := `
		WITH stop_data AS (
			SELECT
				COALESCE(NULLIF(st.stop_headsign, ''), t.trip_headsign, 'No headsign') AS trip_headsign,
